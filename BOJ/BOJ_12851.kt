...
class BOJ_12851 {
    fun solution() {
        ...
        while (queue.isNotEmpty()) {
            ...
            isVisited[distance.value] = true
            listOf(distance.value - 1, distance.value + 1, distance.value * 2).forEach {
        ...
        while (queue.isNotEmpty()) {
            val distance = queue.poll()
            if (min != distance.count) break
            if (distance.value == targetPosition) count++
        }
        println(min)
        println(count)
    }
