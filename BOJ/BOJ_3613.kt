...
class BOJ_3613 {
    fun solution() {
        ...
        when {
            value.isCPP() -> println(value.toJava())
            value.isJava() -> println(value.toCPP())
            else -> println("Error!")
        }
    }

    private fun String.isJava(): Boolean = when {
        this.contains('_') -> false
        this.first().isUpperCase() -> false
        else -> true
    }

    private fun String.toJava(): String {
        val queue = LinkedList(this.toCharArray().toList())
    ...
    private fun String.isCPP(): Boolean = when {
        this.contains(UPPER_CASE_REGEX) -> false
        this.contains("__") -> false
        this.last() == '_' -> false
        this.first() == '_' -> false
        else -> true
    }

    private fun String.toCPP(): String {
        val queue = LinkedList(this.toCharArray().toList())
        val stringBuilder = StringBuilder()
        while (queue.isNotEmpty()) {
            val char = queue.poll()
            if (char.isUpperCase()) {
                stringBuilder.append("_").append(char.toLowerCase())
                continue
            }
            stringBuilder.append(char)
        }
    ...
    companion object {
        private val UPPER_CASE_REGEX = Regex("[A-Z]")
    }
}
