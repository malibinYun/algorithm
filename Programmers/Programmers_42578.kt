...
fun main() {
    ...
    val clazz = Programmers_42578()

    println(clazz.solution(stub1))
    println(clazz.solution(stub2))

    //println(stub1.groupBy { it[1] }.values.fold(1) { acc, value -> acc * (value.size + 1) } - 1)
}
...
class Programmers_42578 {
    fun solution(clothes: Array<Array<String>>): Int {
        ...
        for (cloth in clothes) {
            clothesMap[cloth[1]]?.let {
                clothesMap[cloth[1]] = it + 1
            } ?: run {
                clothesMap[cloth[1]] = 1
            }
        }

        val values = clothesMap.values.map { i -> i + 1 }
        for (value in values) {
            answer *= value
        }
        return answer - 1
...
// 리얼 미친 어썸코드 ㅋㅋ 이래야 코틀린 제대로 쓰는건데..
//fun solution(clothes: Array<Array<String>>): Int {
