...
fun main() {
    val clazz = Programmers_64064()
    val userIds = arrayOf("frodo", "fradi", "crodo", "abc123", "frodoc")
    val bannedIds1 = arrayOf("fr*d*", "abc1**")
    val bannedIds2 = arrayOf("*rodo", "*rodo", "******")
    val bannedIds3 = arrayOf("fr*d*", "*rodo", "******", "******")
    println(clazz.solution(userIds, bannedIds1))
    println(clazz.solution(userIds, bannedIds2))
    println(clazz.solution(userIds, bannedIds3))
}

class Programmers_64064 {
    var matches = mutableSetOf<Set<String>>()
    ...
    private fun search(
        userIds: List<String>,
        bannedIdRegexes: List<Regex>,
        matchedIds: List<String> = listOf()
    ) {
        if (bannedIdRegexes.isEmpty()) {
            matches.add(matchedIds.toSet())
            println(matches)
            return
        }
        if (userIds.isEmpty()) return
        val bannedIdRegex = bannedIdRegexes[0]
        for (userId in userIds) {
            if (bannedIdRegex.matches(userId)) {
                search(userIds - userId, bannedIdRegexes.drop(1), matchedIds + userId)
            }
        }
    }
}
