...
//수정해야 할 소스 파일이 너무 많아서 고민하던 콘은 소스 코드에 작성된 모든 괄호를 뽑아서 올바른 순서대로 배치된 괄호 문자열을 알려주는 프로그램을 다음과 같이
// 개발하려고 합니다.
//
//용어의 정의
...
class Kakao2020_02 {
    ...
    fun split(str: String): String {
        ...
        for (i in 0 until str.length step 2) {
            val left = str.substring(0, i + 2)
            if (isBalanced(left)) {
                val right = str.substring(i + 2, str.length)
                println("left : $left, right : $right")

                if (isPerfect(left)) {
                    return "$left${split(right)}"
                }
                return "(${split(right)})${reverse(left.substring(1, left.lastIndex))}"
            }
        }
        return ""
    }
    ...
    fun isPerfect(str: String): Boolean {
        val stack = Stack<Char>()
        stack.push('$')
